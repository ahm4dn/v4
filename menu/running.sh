...
haproxy=$(systemctl status haproxy | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
if [[ $haproxy == "running" ]]; then 
   shaproxy=" ${GREEN}Running ${NC}( No Error )${NC}"
else
   shaproxy="${RED}  Not Running ${NC}  ( Error )${NC}"
fi
